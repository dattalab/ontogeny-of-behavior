...
# define the functions to apply to the da features
...
def window_trials_pro(
    ...
    signal_matrix = _align_matrix(
        data[signal_key].to_numpy(), trial_indices, window, zscore=zscore_signal
    )
    ...
    mask = ((trial_indices + window[0]) > 0) & (
        (trial_indices + window[-1]) < len(data)
    )
    agg_df = pd.DataFrame(
        {
            "mu": _mu,
    ...
    trial_df = pd.DataFrame(
        {
            ...
            "aligned_syllable": np.repeat(
                _labels.astype("int16"), signal_matrix.shape[1]
            ),
            "trials": trials,
        }
    )
